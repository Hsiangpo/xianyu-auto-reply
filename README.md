...
```python
# db_manager.py 管理员初始化（节选）
default_password = os.getenv("ADMIN_PASSWORD")
if not default_password:
    raise RuntimeError("ADMIN_PASSWORD 未设置")
...
# 管理员配置
ADMIN_USERNAME=admin                   # 管理员用户名（可选，默认 admin；设置后启动时会尝试重命名旧 admin 账号）
ADMIN_PASSWORD=your-strong-password    # 管理员密码（必填）
JWT_SECRET_KEY=your-long-secret-key    # JWT密钥（必填）
PASSWORD_ENCRYPTION_KEY=               # 可选：登录密码加密密钥（未设置时会使用 JWT_SECRET_KEY 派生）
...
> 💡 **提示**：除 ADMIN_PASSWORD/JWT_SECRET_KEY 外，其余配置项都有默认值，可根据需要选择性配置
...
#### 管理员密码
- **用户名**：`admin`
- **密码来源**：`ADMIN_PASSWORD` 环境变量
- **初始化机制**：首次创建数据库时自动创建admin用户；未设置 ADMIN_PASSWORD 会拒绝初始化
- **密码同步**：若数据库已存在，修改 `ADMIN_PASSWORD` 并重启后会自动同步管理员密码（以环境变量为准）；为避免残留不一致，管理员登录时也会进行一次同步兜底
